...
describe('project', () => {
  test('initial state projects with expected basics', () => {
    ...
    expect(ui.buildingTypes).toHaveLength(13)
  ...
  test('research fields are forwarded into UiState', () => {
    ...
    expect(ui.researchProgress.Housing.level).toBe(1)
    expect(ui.researchProgress.Consumer.level).toBe(1)
    expect(ui.researchProgress.Industry.level).toBe(1)
    expect(ui.researchProgress.Energy.level).toBe(1)
  })
})
